        ...
        for epoch in range(self.config.grpo_epochs):
            ...
            if hasattr(self, 'grad_scaler') and self.grad_scaler is not None:
                ...
                if torch.isnan(grad_norm) or torch.isinf(grad_norm):
                    print(f"  WARNING: NaN/Inf gradient detected (norm={grad_norm}), skipping optimizer step but updating scaler")
                    self.optimizer.zero_grad()
                    # CRITICAL FIX: We MUST call update() even if we skip step(), 
                    # otherwise the scaler thinks we're still in the same iteration 
                    # and throws "unscale_() has already been called" next time.
